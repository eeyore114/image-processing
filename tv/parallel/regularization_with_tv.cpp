...
void RegurarizationWithTV(std::vector<float> &proj_img, std::vector<float> &bproj_img, struct Condition cond)
{
	...
	for(int update_count = 0; update_count < cond.update_count; update_count++)
	{
		...
		mlem(bproj_img, proj_img, bproj_img_with_tv, cond);
		writeRawFile("./result/test/after_mlem_float_128-128.raw", bproj_img_with_tv);
		...
		writeRawFile("./result/test/v_float_128-128.raw", v);
		float a = 0.5;
...
void mlem(std::vector<float> &f, std::vector<float> &g, std::vector<float> &h, struct Condition cond)
{
	...
	writeRawFile("./result/test/cij_proj_float_128-20.raw", cij_proj);
	writeRawFile("./result/test/cij_float_128-128.raw", cij);

	ParallelProjecion(f, f_proj, cond);
	writeRawFile("./result/test/f_proj_float_128-20.raw", f_proj);

	for(int i = 0; i < ratio_gf.size(); i++) { ratio_gf[i] =  g[i] / f_proj[i]; }
	writeRawFile("./result/test/ratio_gf_float_128-20.raw", ratio_gf);

	ParallelProjecion(ratio_gf, r_bproj, cond, is_inverse);
	writeRawFile("./result/test/r_bproj_float_128-128.raw", r_bproj);

	for(int i = 0; i < h.size(); i++) { h[i] = (r_bproj[i] * f[i])  / cij[i]; }
	writeRawFile("./result/test/h_float_128-128.raw", h);
