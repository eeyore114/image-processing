...
void set_projection_line_on_collimator();
...
int main()
{
	...
	set_projection_line_on_collimator();
	exit(0);
...
void set_projection_line_on_collimator()
{
	...
	Eigen::Vector3f pinhole_ctr; pinhole_ctr << 17., -11., 4.;
	float theta_xy = 0.;
	float theta_zx = 0.;
	float degree_to_rad = M_PI / 180.;
	Eigen::Vector3f on_collimator;  on_collimator << 0., 0., 0.;
	...
	std::cout << "on_collimator_rot(0) = " << on_collimator_rot(0) << std::endl;
	std::cout << "on_collimator_rot(1) = " << on_collimator_rot(1) << std::endl;
	std::cout << "on_collimator_rot(2) = " << on_collimator_rot(2) << std::endl;
