...
void plot_proj(Condition cond)
{
	...
	readRawFile("./img/primary_detector_float_512-256-90.raw", circle_proj);
	vector<float> square_proj(circle_proj.size());
	readRawFile("./img/primary_detector_rectangle_float_512-256-90.raw", square_proj);
...
{
	// 拡張子を抜いた名前
	string write_profile_name = "ML-EM_100_7ray";
	vector<float> circle_reconst(cond.img_w * cond.img_h * cond.img_d);
	readRawFile("./img/ML-EM100_circle_7ray_float_128-128-128.raw", circle_reconst);
	vector<float> square_reconst(circle_reconst.size());
	readRawFile("./img/ML-EM100_square_7ray_float_128-128-128.raw", square_reconst);
