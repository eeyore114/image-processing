...
int main()
{
    ...
    for(int i = 0; i < N; i++)
    {
	    ...
	    float m = random(mt);
	    float x = cbrt(radius) * sqrt(1 - m * m) * cos(phi);
	    float y = cbrt(radius) * sqrt(1 - m * m) * sin(phi);
	    float z = cbrt(radius) * m;
	 ...
	 writeRawFile("within_sphere_weight_float_128-128-128.raw", H * W * L, f);
