...
int main()
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(medium_num == 1) 	 { sprintf(writeFileName, "cylinder_ca_single_pinhole_%02d_double_180-180.raw", i); }
		else if(medium_num == 2) { sprintf(writeFileName, "cylinder_h2o_single_pinhole_%02d_double_180-180.raw", i); }
		else 					 { sprintf(writeFileName, "cylinder_multi_single_pinhole_%02d_double_180-180.raw", i); }
						...
						while(1)
						{
							...
							else // 超えた場合
							{
								...
								for(int theta_degree = 0; theta_degree < 360; theta_degree += 2)
								{
									...
									if(abs(y_on_collimator) > width_collimator + height_collimator / 2. * tan(M_PI / 6)) { continue; }
									...
									y_on_collimator = past_rotated(1) + vec_scale * (curr_rotated(1) - past_rotated(1));
									if(abs(y_on_collimator) > width_collimator) { continue; }


									//検出器のピクセルサイズが0.5 cmであるため ×2をしている
									// yの値が大きい→検出器の番号は小さい
									float y_on_detector = y_on_collimator - distance_collimator_to_detector * tan_collimator * 10;
