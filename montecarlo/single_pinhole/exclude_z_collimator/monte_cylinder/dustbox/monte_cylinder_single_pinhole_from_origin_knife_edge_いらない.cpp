...
int main()
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(medium_num == 1) 	 { sprintf(writeFileName, "cylinder_ca_single_pinhole_from_origin_%02d_double_65-180.raw", i); }
		else if(medium_num == 2) { sprintf(writeFileName, "cylinder_h2o_single_pinhole_from_origin_%02d_double_65-180.raw", i); }
		else 					 { sprintf(writeFileName, "cylinder_multi_single_pinhole_from_origin_%02d_double_65-180.raw", i); }
	...
	if(medium_num == 1)
	{
		write_energy_spectrum_name = "energy_ca_cylinder_single_pinhole_from_origin.csv";
		write_image_profile_name = "position_ca_cylinder_single_pinhole_from_origin.csv";
	}
	else if(medium_num == 2)
	{
		write_energy_spectrum_name = "energy_h2o_cylinder_single_pinhole_from_origin.csv";
		write_image_profile_name = "position_h2o_cylinder_single_pinhole_from_origin.csv";
	}
	else
	{
		write_energy_spectrum_name = "energy_multi_cylinder_single_pinhole_from_origin.csv";
		write_image_profile_name = "position_multi_cylinder_single_pinhole_from_origin.csv";
	}
...
void RaySimulation(double* energy_spectrum,double* detector, float* cylinder, float scale_ratio_fantom)
{
	...
	for(int m = 0; m < photon_num; m++)
	{
		...
		while(1)
		{
			int j_isair = round(curr_(0) / scale_ratio_fantom + (mumap_size - 1) / 2.0);
			int i_isair = round((mumap_size - 1) / 2.0 - (curr_(1) / scale_ratio_fantom));
			int k_isair = round((mumap_size - 1) / 2.0 - (curr_(2) / scale_ratio_fantom));

			if(cylinder[k_isair * mumap_size * mumap_size + i_isair * mumap_size + j_isair] < 0.1)
			{
