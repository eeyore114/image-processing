...
int JudgeIsair(Eigen::Vector3f curr_, float* cylinder, float scale_ratio_fantom);
void WriteImage(double* detector);
...
int main()
{
	...
	{
		Cylinder = "mu-map_multi_cylinder_float_128-128-128.raw";
		cout << "medium : h2o & ca" << endl;
	}
	else
	{
		Cylinder = "mu-map_air_cylinder_float_128-128-128.raw";
	...
	int w_collimator_mm = W_COLLIMATOR * 10;
	...
	if(medium_num == 2)
	{
		ostr << "result_h2o/from_voxel/" << w_collimator_mm << "mm_collimator/energy_h2o.csv";
		write_energy_spectrum_name = ostr.str();
		ostr.str("");
		ostr << "result_h2o/from_voxel/" << w_collimator_mm << "mm_collimator/position_h2o.csv";
		write_image_profile_name = ostr.str();
	}
	else if (medium_num == 3)
	{
		ostr << "result_multi/from_voxel/" << w_collimator_mm << "mm_collimator/energy_multi.csv";
		write_energy_spectrum_name = ostr.str();
		ostr.str("");
		ostr << "result_multi/from_voxel/" << w_collimator_mm << "mm_collimator/position_multi.csv";
		write_image_profile_name = ostr.str();
	}
	else
	{
		ostr << "result_air/from_voxel/" << w_collimator_mm << "mm_collimator/energy_air.csv";
		write_energy_spectrum_name = ostr.str();
		ostr.str("");
		ostr << "result_air/from_voxel/" << w_collimator_mm << "mm_collimator/position_air.csv";
		write_image_profile_name = ostr.str();
	}
...
void RaySimulation(double* energy_spectrum,double* detector, float* cylinder, float scale_ratio_fantom)
{
	...
	for(int i = 0; i < MUMAP_SIZE; i++)
  {
  	...
  	for(int j = 0; j < MUMAP_SIZE; j++)
  	{
  		...
  		{
				if(cylinder[i * MUMAP_SIZE * MUMAP_SIZE + MUMAP_SIZE * j + k] > 0.0001)
				{
					for(int m = 0; m < photon_num; m++)
					{
						...
						while(1)
						{
							...
							int isair = JudgeIsair(p.curr_, cylinder, scale_ratio_fantom);
							...
							else
							{
								for(int theta_degree = 0; theta_degree < 360; theta_degree += 2)
								{
									...
									float width_collimator = W_COLLIMATOR;
									...
									// yの値が大きい→検出器の番号は小さい
									float y_on_detector = on_colimator(1) + distance_collimator_to_detector * tan_collimator_xy * 10;
									float z_on_detector = on_colimator(2) + distance_collimator_to_detector * tan_collimator_xz * 10;
...
int JudgeIsair(Eigen::Vector3f curr_, float* cylinder, float scale_ratio_fantom)
{
	...
	// 空気の場合は1を入れる
	if(k > 0 && k < MUMAP_SIZE && i > 0 && i < MUMAP_SIZE && j > 0 && j < MUMAP_SIZE)
		isair = (0.02 > cylinder[k * MUMAP_SIZE * MUMAP_SIZE + i * MUMAP_SIZE + j]);
...
void WriteImage(double* detector)
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(medium_num == 2) { sprintf(writeFileName, "result_h2o/from_voxel/%dmm_collimator/cylinder_h2o_single_pinhole_%02d_double_180-180.raw", w_collimator_mm, i); }
		else if (medium_num == 3) { sprintf(writeFileName, "result_multi/from_voxel/%dmm_collimator/cylinder_multi_single_pinhole_%02d_double_180-180.raw", w_collimator_mm, i); }
		else if (medium_num == 4)  { sprintf(writeFileName, "result_air/from_voxel/%dmm_collimator/cylinder_air_single_pinhole_%02d_double_180-180.raw", w_collimator_mm, i); }

		writeRawFile(writeFileName, D_SIZE_INT * D_SIZE_INT, tmp);
	}

	char writeFileName[100];
	if(medium_num == 2) { sprintf(writeFileName, "result_h2o/from_voxel/%dmm_collimator/cylinder_h2o_single_pinhole_sum_double_180-180.raw", w_collimator_mm); }
	else if (medium_num == 3) { sprintf(writeFileName, "result_multi/from_voxel/%dmm_collimator/cylinder_multi_single_pinhole_sum_double_180-180.raw", w_collimator_mm); }
	else  { sprintf(writeFileName, "result_air/from_voxel/%dmm_collimator/cylinder_air_single_pinhole_sum_double_180-180.raw", w_collimator_mm); }
