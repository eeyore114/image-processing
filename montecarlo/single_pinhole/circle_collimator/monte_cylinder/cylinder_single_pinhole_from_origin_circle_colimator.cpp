...
int main()
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(medium_num == 2) { sprintf(writeFileName, "result_h2o/from_origin/cylinder_h2o_single_pinhole_from_origin_%02d_double_180-180.raw", i); }
		else if (medium_num == 3) { sprintf(writeFileName, "result_multi/from_origin/cylinder_multi_single_pinhole_from_origin_%02d_double_180-180.raw", i); }
		else if (medium_num == 4)  { sprintf(writeFileName, "result_air/from_origin/cylinder_air_single_pinhole_from_origin_%02d_double_180-180.raw", i); }
	...
	if(medium_num == 2) { writeFileName = "result_h2o/from_origin/cylinder_h2o_single_pinhole_from_origin_sum_double_180-180.raw"; }
	else if (medium_num == 3) { writeFileName = "result_multi/from_origin/cylinder_multi_single_pinhole_from_origin_sum_double_180-180.raw"; }
	else  { writeFileName = "result_air/from_origin/cylinder_air_single_pinhole_from_origin_sum_double_180-180.raw"; }
	...
	if(medium_num == 2)
	{
		write_energy_spectrum_name = "result_h2o/from_origin/energy_h2o.csv";
		write_image_profile_name = "result_h2o/from_origin/position_h2o.csv";
	}
	else if (medium_num == 3)
	{
		write_energy_spectrum_name = "result_multi/from_origin/energy_multi.csv";
		write_image_profile_name = "result_multi/from_origin/position_multi.csv";
	}
	else
	{
		write_energy_spectrum_name = "result_air/from_origin/energy_air.csv";
		write_image_profile_name = "result_air/from_origin/position_air.csv";
	}
		...
		while(1)
		{
			...
			else
			{
				for(int theta_degree = 0; theta_degree < 360; theta_degree += 2)
				{
					...
					// yの値が大きい→検出器の番号は小さい
					float y_on_detector = on_colimator(1) + distance_collimator_to_detector * tan_collimator_xy * 20;
					float z_on_detector = on_colimator(2) + distance_collimator_to_detector * tan_collimator_xz * 20;
