...
void launchProjection(std::vector<float> &init_img, std::vector<float> &detector, Condition cond)
{
	...
	printf("cal_fov\n");
	create_fov(fov, cond);
	std::vector<float> fov_float(fov.size());

	for(int i = 0; i < fov.size(); i++) fov_float[i] = fov[i];

	writeRawFile("fov_gradient_float_512-1.raw", fov_float);
	...
	// 画像書き込み
	writeRawFile("detector_float_512-180.raw", detector);
	...
	writeRawFile("reconstruct_img_float_128-128.raw", reconstruct_img);
		...
		for(int m = 0; m < cond.detector_size_w; m++)
		{
			for(int pinhole_num = 0; pinhole_num < cond.pinhole_count; pinhole_num++)
			{
				...
				for (int sample_point = 0; sample_point < 300; sample_point++)
				{
					...
					if(is_inverse)
					{
						if(isnan(g[index1]) || g[index2] || g[index3] || g[index4])
						{
							// ここら辺でバグってる（IJをみてみる）
...
void create_fov(std::vector<int> &fov, Condition cond)
{
	...
	for(int pinhole = 0; pinhole < cond.pinhole_count; pinhole++)
	{
		...
		Eigen::Vector2f gradient_vec;
		// origin << 0., 0., 0.;
		// gradient_vec = calculate_unit_vector(on_collimator, origin);
		...
		for(int i = 0; i < cond.detector_size_w; i++)
		{
			...
			float theta_max = acos(max.dot(base));
			float theta = acos(vec.dot(base));
			if(max.dot(base) + gradient_vec.dot(base) > vec.dot(base)) { continue; }
			// if(theta_max > theta) { continue; }
			fov[i] = pinhole;
		}
	}
