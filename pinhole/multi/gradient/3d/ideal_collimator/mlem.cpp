...
int main()
{
	...
	cond.distance_collimator_to_detector = 7.;
...
void launchProjection(std::vector<float> &init_img, std::vector<float> &detector, Condition cond)
{
	std::vector<float> pinhole_x{ -10.7f, 0., 10.7f, -5., 5., -10.7f,  0., 10.7f };
	std::vector<float> pinhole_z{ 	3.8f, 4.,  3.8f,  0., 0., -3.8f, -4., -3.8f };
...
void launchBackProjection(std::vector<float> &detector, std::vector<float> &reconstruct_img, Condition cond)
{
	std::vector<float> pinhole_x{ -10., 0., 10., -5., 5., -10.,  0., 10. };
	std::vector<float> pinhole_z{ 	5., 4.,  5.,  0., 0., -5., -4., -5. };
	...
	for(int theta_degree = 0; theta_degree < 360; theta_degree += delta_detector)
	{
		...
		for(int m = 0; m < cond.detector_size_h; m++)
		{
			for(int n = 0; n < cond.detector_size_w; n++)
			{
				...
				for(int pinhole_num = 0; pinhole_num < pinhole_x.size(); pinhole_num++)
				{
...
void create_fov(std::vector<int> &fov, std::vector<float> &pinhole_x, std::vector<float> &pinhole_z, Condition cond)
{
	...
	for(int pinhole_num = 0; pinhole_num < pinhole_x.size(); pinhole_num++)
	{
		...
		Eigen::Vector3f origin; origin << 0., 0., 0.;
		Eigen::Vector3f base = calculate_unit_vector(origin, on_collimator);
		Eigen::Matrix3f rot3d;
		axis << 0, 0, 1;
		rot3d = Eigen::AngleAxisf(cond.fov_theta, axis);
		Eigen::Vector3f max_vec = base;
		max_vec = rot3d * base;
