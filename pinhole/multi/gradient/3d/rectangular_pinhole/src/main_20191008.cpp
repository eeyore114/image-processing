...
void validate_rectangular_pinhole_geometry(Condition cond, std::vector<float> pinhole_center, std::vector<float> pinhole_theta_xy, std::vector<float> pinhole_theta_zx);
...
void validate_rectangular_pinhole_geometry(Condition cond, std::vector<float> pinhole_center, std::vector<float> pinhole_theta_xy, std::vector<float> pinhole_theta_zx)
{
	...
	float y_pillar = collimator_radius_y / tan(aperture_xy);
	float z_pillar = collimator_radius_z / tan(aperture_zx);
	if(abs(y_pillar - z_pillar) < 0.1) return;

	cout << "error : geometry is incorrect" << endl;
}

void set_pinhole_img(std::vector<int> &pinhole_img_layer1, std::vector<int> &pinhole_img_layer3, std::vector<int> &fov, Condition cond, std::vector<float> pinhole_center, std::vector<float> pinhole_theta_xy, std::vector<float> pinhole_theta_zx)
{
	validate_rectangular_pinhole_geometry(cond, pinhole_center, pinhole_theta_xy, pinhole_theta_zx);
...
void create_pinhole_img(std::vector<int> &img, Condition cond, Eigen::Vector3f pinhole_center, float pinhole_theta_xy, float pinhole_theta_zx, int pinhole_num, int option)
{
	...
	*/
	...
	- 角の座標求める
			- 中心で回転行列で回転
			- コリメータの中心に移動させる
	- layer3 → 角の座標のベクトルの4つの交点を求める（回転前の座標の方がいいかも）
	- 交点を回転
	- 交点 → 角の座標（回転後）の座標で求めたベクトルでlayer3の座標を求める
		
	*/
	int corner_count = 4;
	// 順番：左上，右上，左下，右下
	std::vector<float> pinhole_corner(cond.pinhole_count * coord_num * corner_count);
	rep(p_num, cond.pinhole_count)
	{
		rep(i, corner_count) pinhole_corner[p_num * coord_num * corner_count + i * coord_num + X] = pinhole_center(X);

		pinhole_corner[p_num * coord_num * corner_count + 0 * coord_num + Y] = pinhole_center(Y) - cond.collimator_w_y_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 1 * coord_num + Y] = pinhole_center(Y) + cond.collimator_w_y_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 2 * coord_num + Y] = pinhole_center(Y) - cond.collimator_w_y_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 3 * coord_num + Y] = pinhole_center(Y) + cond.collimator_w_y_axis / 2.;

		pinhole_corner[p_num * coord_num * corner_count + 0 * coord_num + Z] = pinhole_center(Z) + cond.collimator_w_z_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 1 * coord_num + Z] = pinhole_center(Z) + cond.collimator_w_z_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 2 * coord_num + Z] = pinhole_center(Z) - cond.collimator_w_z_axis / 2.;
		pinhole_corner[p_num * coord_num * corner_count + 3 * coord_num + Z] = pinhole_center(Z) - cond.collimator_w_z_axis / 2.;
	}
	// set_corner_coordinate();
	// set_intersection_of_4corner();
	// 回転
	// layer, fovの座標を求める

	/*--------------------- ピンホールの形が正しくなっていないため，修正  end  -------------------------*/
