...
#define  PN  9     /* number of parameters + 1 */
...
typedef struct {
	...
	double  u;      /* frequency of x-direction */
	double  v;      /* frequency of y-direction */
	double  p;      /* initial phase */
} Param;

char *menu[PN] = {
	"sine and cosine plane wave",
	"Output sin image file name <float> ",
	"Output cos image file name <float> ",
	"Number of width      ",
	"Number of height     ",
	"amplitude of wave        ",
	"frequency of x-direction ",
	"frequency of y-direction ",
	"initial phase            ",
};

void write_data(char *, float *, int);
void plane_wave(float *, float *, int, int, double, double, double, double);
...
void getparameter(int argc, char **argv, Param *pm)
{
	...
	sprintf( pm->f1, "psin.img");
	sprintf( pm->f2, "pcos.img");
	pm->nx = 128;
	pm->ny = 128;
	pm->a = 1;
	pm->u = 5;
	...
	if( argc == 1+i ) {
		...
		fprintf( stdout, " %s [%f] :", menu[i++], pm->v );
		if(*gets(dat) != '\0')  pm->v = atof(dat);
...
main(int argc, char *argv[] )
{
	...
	printf(" *** Make Sine & Cosine plane wave Image ***\n");
	plane_wave(pm->simg, pm->cimg, pm->nx, pm->ny, pm->a, pm->u, pm->v, pm->p);
...
void plane_wave(float *simg, float *cimg, int nx, int ny, double a, double u, double v, double p)
{
	int    i, j;
	double x, y;
	...
	for(i = 0 ; i < ny ; i++) {
		...
		for(j = 0 ; j < nx ; j++) {
			x = j-nx/2;
			simg[i*nx+j] = (float)(a*sin(2*PI*(u*x+v*y)/nx+p));
			cimg[i*nx+j] = (float)(a*cos(2*PI*(u*x+v*y)/nx+p));
		}
	}
}
