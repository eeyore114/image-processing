...
int main()
{
	...
	{
		create_mu_map_h2o(f);
		writeRawFile("mu-map_h2o_cylinder_float_128-128-128.raw", H * W * D, f);
	}

	// 複数媒質のmumap
	// {
	// 	create_mu_map_multi(f);
	// 	writeRawFile("mu-map_multi_cylinder_float_128-128-128.raw", H * W * D, f);
	// }

	// airのmumap
	// {
	// 	create_mu_map_air(f);
	// 	writeRawFile("mu-map_air_cylinder_float_128-128-128.raw", H * W * D, f);
	// }
	...
	for(int i = 0; i < D; i++)
	{
		for(int j = 0; j < H; j++)
		{
			for(int k = 0; k < W; k++)
			{
				...
				x *= ratio;
				y *= ratio;
				if(pow(x, 2.) + pow(y, 2.) < pow(10, 2.)) { f[i * W * H + j * W + k] = 0.15; }
			}
		}
	}
		...
		for(int i = 0; i < D; i++)
		{
			for(int j = index1; j < index1 + small_size; j++)
			{
				for(int k = index2; k < index2 + small_size; k++)
				{
					...
					if(pow(small_x, 2.) + pow(small_y, 2.) < pow(r / ratio, 2.)) { f[i * W * H + j * W + k] = 0.28; }
				}
			}
		}
	...
	for(int i = 0; i < D; i++)
	{
		for(int j = 0; j < D; j++)
		{
			for(int k = 0; k < D; k++)
			{
				...
				x *= 0.2;
				y *= 0.2;
				if(pow(x, 2.) + pow(y, 2.) < pow(10, 2.)) { f[i * W * H + j * W + k] = 0.01; }
			}
		}
	}
}
